...
export default function EditCustomerPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    params.then(({ id: resolvedId }) => {
      ...
      Promise.all([
        supabase.from("customers").select("name, phone, line_user_id").eq("id", resolvedId).eq("salon_id", DEFAULT_SALON_ID).single(),
        supabase.from("pets").select("id, name, breed, birth_date, weight_kg, rabies_vaccination_date, notes").eq("customer_id", resolvedId).limit(1).maybeSingle(),
      ]).then(([{ data: c }, { data: p }]) => {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (cErr) {
      setError("飼い主情報の更新に失敗しました: " + cErr.message);
      setSaving(false);
      return;
    }

    if (petId) {
      const { error: pErr } = await supabase.from("pets").update({
        name: pet.name.trim(),
      ...
      if (pErr) {
        setError("ペット情報の更新に失敗しました: " + pErr.message);
        setSaving(false);
        return;
      }
    }
  ...
  if (loading) {
    return (
      <div className="max-w-2xl mx-auto py-8 px-4">
        <div className="text-sm" style={{ color: "var(--ink-soft)" }}>読み込み中…</div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-2xl mx-auto py-8 px-4">
      ...
      <h1 className="font-display text-3xl font-semibold tracking-tight mb-8">顧客情報を編集</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {/* Pet */}
        <div className="card p-6">
          <h2 className="font-display text-lg font-semibold mb-4">ペット情報{!petId && <span className="text-xs font-normal ml-2" style={{ color: "var(--ink-soft)" }}>(未登録)</span>}</h2>
          <div className="space-y-4">
            <div>
